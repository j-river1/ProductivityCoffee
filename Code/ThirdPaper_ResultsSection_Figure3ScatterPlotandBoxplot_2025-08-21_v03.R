...
pdp_FarmUseCoffee_CT   <- rbind(
  model.rf.smarthphone %>%  partial(pred.var=c("FarmUseCoffee"), approx=T) %>% cbind(., Monitoring = "RF Smarthphone monitoring"),
  model.rf.manual %>%  partial(pred.var=c("FarmUseCoffee"), approx=T)  %>% cbind(., Monitoring = "RF Manual monitoring")
) 
pdp_FarmUseCoffee_CT$Monitoring <- factor(pdp_FarmUseCoffee_CT$Monitoring, levels=c("RF Smarthphone monitoring","RF Manual monitoring"))


pdp_FarmArea_CT    <- rbind(
  model.rf.smarthphone %>%  partial(pred.var=c("FarmArea"), approx=T) %>% cbind(., Monitoring = "RF Smarthphone monitoring"),
  model.rf.manual %>%  partial(pred.var=c("FarmArea"), approx=T)  %>% cbind(., Monitoring = "RF Manual monitoring")
)
pdp_FarmArea_CT$Monitoring <- factor(pdp_FarmArea_CT$Monitoring, levels=c("RF Smarthphone","RF Manual monitoring"))


Fig05a <-
  ggplot(pdp_FarmUseCoffee_CT , aes(x=FarmUseCoffee, y=yhat, color= Monitoring)) +
  geom_line(size=1) +
  geom_vline(xintercept = 12.8, color = "red", linetype = "dashed", size = 1) + 
  scale_color_manual(values=c("RF Smarthphone monitoring"="darkgreen",
                              "RF Manual monitoring"="darkblue"))  +
...
data_train_smarthphone$Monitoring <- "Smartphone monitoring"
data_train_manual$Monitoring  <- "Manual monitoring"
...
Fig05b <-ggplot(data_combined, aes(x = FarmUseCoffee, fill = Monitoring)) +
  ...
  scale_fill_manual(values = c(
    "Smartphone monitoring" = "darkgreen",
    "Manual monitoring" = "darkblue"
  )) +
  theme_bw() +
...
Fig05c <-
  ...
  geom_line(size=1) +
  scale_color_manual(values=c("RF Smartphone monitoring"="darkgreen",
                              "RF Manual monitoring"="darkblue"))  +
  ylab("Partial dependence") + xlab("")+
...
Fig05 <- Fig05a + Fig05b +
  plot_annotation(tag_levels = "a") +
  plot_layout(heights=c(9,1), widths = c(0.1))

ggsave(paste0("results/Figure3_partialdepencePlot_2025-08-21_v06.jpg"), plot = Fig05 , dpi = 300, width = 8, height = 6)
